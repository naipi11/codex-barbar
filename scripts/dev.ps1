...
<#
.SYNOPSIS
    Build and run the codex-barbar Tauri desktop shell for Windows.
...
$DesktopBinaryName = "codex-barbar.exe"
...
if (-not $SkipBuild) {
    ...
    try {
        if ($Release) {
            Write-Host "Building codex-barbar (release, no bundle)..." -ForegroundColor Cyan
            & $pnpmCommand.Source run tauri:build
        } else {
            Write-Host "Building codex-barbar (debug, no bundle)..." -ForegroundColor Cyan
...
Write-Host ""
Write-Host "Starting codex-barbar..." -ForegroundColor Green
& $binary
