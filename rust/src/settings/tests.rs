...
fn test_legacy_per_provider_fields_migrate_into_provider_configs() {
    ...
    // Cookie sources
    assert_eq!(settings.cookie_source(ProviderId::Codex), "appServer");
    ...
    assert_eq!(settings.usage_source(ProviderId::Codex), "appServer");
...
fn test_provider_configs_roundtrip() {
    ...
    let loaded: Settings = serde_json::from_str(&json).unwrap();
    assert_eq!(loaded.cookie_source(ProviderId::Codex), "appServer");
    ...
    let codex_config = settings
        .provider_configs
        .get(&ProviderId::Codex)
        .expect("Codex keeps non-auth display settings");
    assert!(codex_config.cookie_source.is_none());
    assert!(codex_config.usage_source.is_none());
    assert!(codex_config.api_region.is_none());
    assert!(codex_config.manual_cookie_header.is_none());
    assert!(codex_config.api_token.is_none());
}
...
#[test]
fn shipping_settings_path_derives_from_canonical_app_paths() {
    let paths =
        crate::app_paths::AppPaths::from_local_app_data(Path::new(r"C:\Users\A\AppData\Local"));
    assert_eq!(
        Settings::shipping_settings_path(&paths),
        paths.root.join("settings.json")
    );
}

#[test]
fn codex_legacy_auth_and_source_settings_are_ignored() {
    let json = r#"{
        "provider_configs": {
            "codex": {
                "cookie_source": "browser",
                "usage_source": "oauth",
                "manual_cookie_header": "session=PLACEHOLDER",
                "api_token": "token-placeholder",
                "api_region": "https://legacy.invalid"
            }
        }
    }"#;
    let settings: Settings = serde_json::from_str(json).unwrap();
    ...
    assert_eq!(settings.cookie_source(ProviderId::Codex), "appServer");
    assert_eq!(settings.usage_source(ProviderId::Codex), "appServer");
    assert_eq!(settings.manual_cookie_header(ProviderId::Codex), "");
    assert_eq!(settings.api_token(ProviderId::Codex), "");

    let serialized = serde_json::to_string(&settings).unwrap();
    let codex_config = serialized
        .find("\"codex\":")
        .map(|index| &serialized[index..])
        .unwrap_or_default();
    assert!(
        !codex_config.contains("cookie_source")
            && !codex_config.contains("usage_source")
            && !codex_config.contains("manual_cookie_header")
            && !codex_config.contains("api_token")
            && !codex_config.contains("api_region"),
        "Codex legacy setting leaked into serialized output: {serialized}"
    );
    assert!(
        !serialized.contains("\"codex\":{}"),
        "empty Codex provider config should be removed: {serialized}"
    );
}

#[test]
fn shipping_load_normalizes_provider_lists_to_codex_only() {
    let temp = tempfile::tempdir().expect("temp dir");
