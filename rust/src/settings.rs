...
pub struct Settings {
    ...
    /// Auxiliary window display scale, in the inclusive range 100..=250.
...
/// The only usage path available to the Codex-only V1 product.
const CODEX_APP_SERVER_SOURCE: &str = "appServer";

/// Default API region for providers that expose one.
fn default_api_region(id: ProviderId) -> &'static str {
    match id {
        ProviderId::Alibaba => "singapore",
...
impl Settings {
    ...
    /// Settings path for the codex-barbar V1 desktop product.
    ///
    /// Lives under the canonical `%LOCALAPPDATA%\codex-barbar` root so the
    /// desktop never reads or writes the legacy `CodexBar` roaming store.
    pub fn shipping_settings_path(paths: &crate::app_paths::AppPaths) -> PathBuf {
    ...
    /// Restrict the enabled-provider set and display order to the Codex-only
    /// shipping registry.
    pub fn normalize_shipping_provider_lists(&mut self) {
        self.enabled_providers = crate::core::shipping_provider_ids()
            .iter()
            .map(|id| id.cli_name().to_string())
            .collect();
        self.provider_order = crate::core::shipping_provider_ids()
            .iter()
            .map(|id| id.cli_name().to_string())
            .collect();
        self.sanitize_codex_provider_config();
    }

    /// Remove legacy Codex settings that could otherwise suggest a cookie,
    /// API-token, custom-endpoint, or alternate source path. The provider
    /// itself is App Server-only; these values are retained nowhere after a
    /// settings load/normalization pass.
    pub fn sanitize_codex_provider_config(&mut self) {
        if let Some(config) = self.provider_configs.get_mut(&ProviderId::Codex) {
            config.cookie_source = None;
            config.usage_source = None;
            config.api_region = None;
            config.manual_cookie_header = None;
            config.api_token = None;
            config.gateway_url = None;
            config.workspace_id = None;
            config.ide_base_path = None;
        }
        if self.provider_configs.get(&ProviderId::Codex) == Some(&ProviderConfig::default()) {
            self.provider_configs.remove(&ProviderId::Codex);
        }
    }

    /// Load settings for the codex-barbar V1 desktop product from the
    /// canonical `%LOCALAPPDATA%\codex-barbar` settings file, normalizing
    /// the provider selection to the Codex-only shipping registry.
    pub fn load_shipping() -> Self {
        let paths = match crate::app_paths::AppPaths::discover() {
            Ok(paths) => paths,
            Err(_) => return Self::default(),
        };
        Self::shipping_load_at(&Self::shipping_settings_path(&paths))
    }
    ...
    pub fn provider_config_mut(&mut self, id: ProviderId) -> &mut ProviderConfig {
        let entry = self.provider_configs.entry(id).or_default();
        if id == ProviderId::Codex {
            entry.cookie_source = None;
            entry.usage_source = None;
            entry.api_region = None;
            entry.manual_cookie_header = None;
            entry.api_token = None;
            entry.gateway_url = None;
            entry.workspace_id = None;
            entry.ide_base_path = None;
        }
        entry
    }
