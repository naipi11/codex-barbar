...
// matches the Rust V1_LOCALE_KEYS list in rust/src/locale.rs exactly
...
// Extract the V1 desktop key strings from rust/src/locale.rs.
const rustKeysMatch = rustSrc.match(
  /pub const V1_LOCALE_KEYS:\s*&\[&str\]\s*=\s*&\[([\s\S]*?)\];/,
);
if (!rustKeysMatch) {
  die(2, "could not locate `V1_LOCALE_KEYS` in rust/src/locale.rs");
}
const rustKeys = [];
const v1KeyRe = /"([^"]+)"/g;
let m;
while ((m = v1KeyRe.exec(rustKeysMatch[1])) !== null) {
  rustKeys.push(m[1]);
}
if (rustKeys.length === 0) {
  die(2, "parsed zero keys from V1_LOCALE_KEYS");
}
