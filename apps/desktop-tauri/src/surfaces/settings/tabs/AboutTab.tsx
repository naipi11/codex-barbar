...
export default function AboutTab({ version, copy }: { version: string; copy: SettingsCopy }) {
  const [busy, setBusy] = useState(false);
  const [result, setResult] = useState<ManualUpdateResult | null>(null);
  const [error, setError] = useState<string | null>(null);
  return <section aria-label={copy.about.title}>
    <h2>{copy.about.title}</h2><p>{copy.about.description}</p><p className="settings-about__version" data-testid="about-installed-version">{copy.about.version(version)}</p><p>{copy.about.license}</p>
